...
module sDTW(input clk,
	...
	logic	[`DATA_OUT_WIDTH-1:0]		counter		[`QUERY_LEN-1:0]; //TBD: check if width can be reduced 
	wire	[`DATA_OUT_WIDTH-1:0]		pe_op 		[`QUERY_LEN-1:0];
	wire	[`DATA_OUT_WIDTH-1:0]		pe_prev_op 	[`QUERY_LEN-1:0];
	
	// first PE declared separately since it has nothing on the left; just constants as inputs
	pe first_col(	.active(active[0]),
					.clk(clk),
					.rst(rst),
					.rst_value (1),
					.left      (0),
